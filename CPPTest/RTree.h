...
public:
	...
	struct Branch
	{
		...
		DATATYPE m_data=NULL;                              ///< Data Id    xuer :Only for the leaf node??
	};
	...
	struct Node
	{
		...
		int m_count=NULL;                                  ///< Count  xuer: number of branches??/Nodes？？？
...
public:
	...
	private:
		...
		struct StackElement //xuer: what is m_branchIndex mean???
		{
			Node* m_node;
			int m_branchIndex;
		};
	...
	public:
		...
		/// Access the current data element. Caller must be sure iterator is not NULL first.
		const DATATYPE& operator*() const //xuer * signal
		{
			ASSERT(IsNotNull());
			StackElement& curTos = m_stack[m_tos - 1];
			return curTos.m_node->m_branch[curTos.m_branchIndex].m_data;
		...
		/// Find the next data element   //xuer  ++
	...
	void GetFirst(Iterator& a_it)
	{
		...
		while (first)
		{
			...
			{
				a_it.Push(first, 1); // Descend sibling branch later  xuer: what 1 mean?
			}
			...
			first = first->m_branch[0].m_child; //xuer: why just index 0???
		}
	}
...
public:
	...
	/*struct Branch
	{
		Rect m_rect;                                  ///< Bounds
		Node* m_child;                                ///< Child node
		DATATYPE m_data;                              ///< Data Id    xuer :Only for the leaf node??
	};*/
	...
	//	int m_count;                                  ///< Count  xuer: number of branches??/Nodes？？？
	...
	struct PartitionVars
	{
		...
		int *m_partition = new int;     // xuer to store the node belong to which group? 0/1????
		int m_total;                       //xuer the number of the rectangle/branches
		int m_minFill;
		int m_count[2];                    //xuer  the number of notes in every group
		Rect m_cover[2];
		ELEMTYPEREAL m_area[2];            // xuer the are of each two group
...
RTREE_QUAL::RTree(int NUMDIMS, int &TMAXNODESs) {//Defualt TMAXNODES=8 TMINNODES = TMAXNODES / 2
	...
	ASSERT(TMAXNODES > TMINNODES);
	ASSERT(TMINNODES > 0);
	cout << "NUMDIMS = " << NUMDIMS << ", TMAXNODES = " << TMAXNODES << ", TMINNODES =" << TMINNODES << endl;
	// Precomputed volumes of the unit spheres for the first few dimensions
	m_root = AllocNode();
	m_root->m_level = 0;
...
RTREE_TEMPLATE
void RTREE_QUAL::CountRec(Node* a_node, int& a_count)           //xuer  count the data of the subtree, not Node???
{
...
typename RTREE_QUAL::Rect RTREE_QUAL::NodeCover(Node* a_node)
{
	...
	Rect rect = a_node->m_branch[0].m_rect;                                          //xuer: What the index=0 mean?????
...
// Calculate the n-dimensional volume of a rectangle      xuer: can get the area of rectangle
RTREE_TEMPLATE
...
// Calculate the n-dimensional APCA Point volume of a rectangle      xuer: can get the area of rectangle
RTREE_TEMPLATE
ELEMTYPEREAL RTREE_QUAL::APCARectVolume(Rect* a_rect)
{
	...
	for (int index = 1; index < NUMDIMS; index++)
	{
		if (index & 1) {//odd
			...
			if (f_temp_difference == 0) continue;
...
void RTREE_QUAL::LoadNodes(Node* a_nodeA, Node* a_nodeB, PartitionVars* a_parVars)
{
	...
	for (int index = 0; index < a_parVars->m_total; ++index)
	{
		...
		//targetNodes[0]->m_branch = new Branch[TMAXNODES];
		//targetNodes[1]->m_branch = new Branch[TMAXNODES];
...
void RTREE_QUAL::PickSeeds(PartitionVars* a_parVars)
{
	...
	for (int indexA = 0; indexA < a_parVars->m_total - 1; ++indexA)    //xuer Bubble sort????????
	{
...
// Delete a data rectangle from an index structure.   //xuer delete the data, not the Node,internalNode??????
...
bool RTREE_QUAL::RemoveRect(Rect* a_rect, const DATATYPE& a_id, Node** a_root)
{
	...
	if (!RemoveRectRec(a_rect, a_id, *a_root, &reInsertList))  //xuer Returns true if record not found, false if success.
	{
...
bool RTREE_QUAL::Search(Node* a_node, Rect* a_rect, int& a_foundCount, std::function<bool(const DATATYPE&)> callback) const
{
	...
	else
	{
		...
		for (int index = 0; index < a_node->m_count; ++index)
		{
			if (Overlap(a_rect, &a_node->m_branch[index].m_rect))
			{
				DATATYPE& id = a_node->m_branch[index].m_data;//xuer id is the index of data, not Node
				++a_foundCount;
